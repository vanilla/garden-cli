...
class Cli {
     ...
     * @return void
     */
    private function addArg(CommandSchema $schema, Args $args, mixed $arg): void {
     ...
     * @param bool   $addSpaces Whether to right-pad the cell with spaces.
     * @return string[] Returns an array of strings representing the lines in the cell.
     *
     * @psalm-return array<int, string>
     */
    public static function breakLines(string $text, int $width, bool $addSpaces = true): array {
        $rawLines = explode("\n", $text);
        $lines = [];
     ...
     * @param int $width The width of the cell.
     * @param bool $addSpaces Whether to right pad the lines with spaces.
     * @return int[]|string[] Returns an array of lines broken on word boundaries.
     *
     * @psalm-return array<int, string>
     */
    ...
    /**
     * Determines whether the schema has a command.
     *
    ...
    public function hasOptions(string $command = ''): bool {
        if ($command) {
            ...
            foreach ($this->commandSchemas as $def) {
                if ($def->hasOpts()) {
                    return true;
                }
            }
    ...
    /**
     * Determines whether a command has args.
     *
    ...
    protected function parseRaw(array $argv = null): Args {
        ...
        if (count($argv)) {
            ...
            // Get the data types for all the commands.
            if (!isset($schema)) {
                $schema = $this->getSchema($parsed->getCommand());
            }
                ...
                if ($str === '--') {
                    ...
                    if (isset($argv[$i + 1])) {
                        ...
                        if ($type === self::TYPE_BOOLEAN) {
                            ...
                            $v = $nextArg;
                            $i++;
                        } // else the next arg is another opt.
                    }
     ...
     * @param mixed|null $value Set a new value for the meta key.
     * @return mixed Returns the current value of the meta item or `$this` for fluent setting.
     */
    public function meta(string $name, mixed $value = null): mixed {
        if ($value !== null) {
            $this->currentSchema->setMeta($name, $value);
            return $this;
        }
        return $this->currentSchema->getMeta($name);
    }
     ...
     * @return $this
     */
    public function opt(string $name, string $description, bool $required = false, string $type = 'string',
                        array $meta = []): static {
     ...
     * @return $this
     */
    public function arg(string $name, string $description, bool $required = false): static {
     ...
     * @return $this
     */
    public function command(string $pattern): static {
     ...
     */
    protected function isStrictBoolean(mixed $value, bool &$boolValue = null): bool {
    ...
    /**
     * Guess whether to format the output with colors.
     *
     ...
     * @throws Exception
     */
    protected function validateType(mixed &$value, string $type, string $name = '', OptSchema $def = null): bool {
    ...
    /**
     * Validate the type of value and coerce it into the proper type.
     *
     ...
     */
    private function validateScalarType(mixed &$value, string $type, string $name = '', ?OptSchema $def = null): bool {
        switch ($type) {
    ...
    /**
     * Writes a lis of all the commands.
     *
     * @return void
     */
     ...
     */
    public static function val(int|string $key, array $array, mixed $default = null): mixed {
     ...
     */
    private function pushOpt(Args $args, string $key, mixed $value): void {
        if ($args->hasOpt($key)) {
     ...
     */
    private function mergeOpt(Args $args, string $key, mixed $value): void {
        if (is_array($value)) {
            $args->setOpt($key, array_merge((array)$args->getOpt($key, []), $value));
        } else {
            $args->setOpt($key, $value);
     ...
     */
    private function extractOpt(array &$opts, string $key, OptSchema $def): mixed {
        $r = [];
        $unset = [];
